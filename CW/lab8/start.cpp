...
int block(int x, int y, vector<vector<bool> > & blocked_grid);
...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y) {
  ...
  //     blocked_grid[a[y].getx()][a[y].gety()] == "&";
  // }
}
...
cout << "no block is " << block(start_x, start_y, blocked_grid) << endl;
print_grid(blocked_grid, start_x, start_y);
  return 0;
}
...
//check2
// int block(int x, int y, vector<vector<bool> > const& blocked_grid){
//     if (x < 0 || y < 0) {return 0;}
//     if (blocked_grid[x][y]){return 0;}

//     if (x == 0 && y == 0) {
//       return 1;
//     }
//     return block(x - 1, y, blocked_grid) + block(x, y - 1, blocked_grid);
// }
