...
template <class T> void Vec<T>::print(const Vec<T>& v){
  std::cout << " alloc " << m_alloc << " size " <<
  m_size << " data " << m_data << std::endl;
}

template <class T> int remove_matching_elements(Vec<T>& v, const T& val){
  int counter = 0;
  int a = v.size();
...
// template <class T> Vec<T>& Vec<T>::operator==(const T& val) const {
//   if (this == val) {
//     return true;
//   }
//   else {
