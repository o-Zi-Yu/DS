...
void SortingTest() {
  ...
  PrintDeckPrimary  ("             order before:",deck);
  CutDeck(deck,top,bottom,"perfect");
  PrintDeckPrimary  ("            top:",top);
  PrintDeckPrimary  ("            bottom:",bottom);
  deck = Shuffle(bottom,top,"perfect");
  PrintDeckPrimary  ("            after:",deck);
  assert (DeckSize(deck) == 13);
  CutDeck(deck,top,bottom,"perfect");
  PrintDeckPrimary  ("            top:",top);
  PrintDeckPrimary  ("            bottom:",bottom);
...
//TEST 1:
//to check if CopyDeck() can copy
//sorted_prev pointers and sorted_next pointers

//this also checks cut and shuffle of odd number of cards
Node* i_am_a_deck = CreateDeck(13);
PrintDeckPrimary("INITIAL:",i_am_a_deck);
Node *top1, *bottom1;
CutDeck(i_am_a_deck,top1,bottom1,"perfect");
PrintDeckPrimary("top1:",top1);
PrintDeckPrimary("bottom1:",bottom1);
i_am_a_deck = Shuffle(bottom1,top1,"perfect");
Node *sorted1 = SortHand(i_am_a_deck);
PrintDeckSorted("                    sorted order:",sorted1);
PrintDeckPrimary  ("primary order after (no change!):",i_am_a_deck);
Node* copy_deck = CopyDeck(i_am_a_deck);
PrintDeckPrimary("Copied initial:",copy_deck);
//I know that in this case the head node will also be the
//front node after sorted (head node is A, so is sorted head)
PrintDeckSorted("Copied sorted:",copy_deck);
DeleteAllCards(i_am_a_deck);
DeleteAllCards(copy_deck);

//TEST 2:
//special case in which there are only three players 
//each with three cards
//and the deck is not fully distributed
Node* deck3 = CreateDeck(16);
  PrintDeckPrimary(" deck:",deck3);
  assert (DeckSize(deck3) == 16);

  Node* hands[3];

  Deal(deck3,hands,3,"one-at-a-time",3);

  PrintDeckPrimary(" west:",hands[0]);
  PrintDeckPrimary("north:",hands[1]);
  PrintDeckPrimary(" east:",hands[2]);
  PrintDeckPrimary(" deck after:",deck3);
