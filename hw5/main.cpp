...
void StudentTests() {
  ...
  //shuffle cards with more in top and more in bottom
  Node* deck2 = CreateDeck(21);
  PrintDeckPrimary  ("            primary order before:",deck2);
  Node *top, *bottom;
  CutDeck(deck2,top,bottom,"perfect");
  deck2 = Shuffle(bottom,top,"perfect");
  CutDeck(deck2,top,bottom,"perfect");
  deck2 = Shuffle(top,bottom,"perfect");
  PrintDeckPrimary  ("            primary order before:",deck2);
  assert(SanityCheckPrimary(deck2));
  DeleteAllCards(deck2);

  //divide 7 cards to 4 players
  Node* deck = CreateDeck(7);
  PrintDeckPrimary(" deck:",deck);

  Node* hands[4];

  Deal(deck,hands,4,"one-at-a-time",2);
  PrintDeckPrimary("west",hands[0]);
  PrintDeckPrimary("north:",hands[1]);
  PrintDeckPrimary(" east:",hands[2]);
  PrintDeckPrimary("south:",hands[3]);
